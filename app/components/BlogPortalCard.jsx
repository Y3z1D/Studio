...
export default function BlogPortalCard() {
  ...
  const timerRefsRef = useRef({ scanTimer: null, verifyTimer: null, routeTimer: null });
  ...
  useEffect(() => {
    ...
    // Add animation class to enable animations
    if (modalRef.current) {
      modalRef.current.classList.add('is-animating');
    }
    ...
    timerRefsRef.current = { scanTimer, verifyTimer, routeTimer };
    ...
    return () => {
      ...
      if (modalRef.current) {
        modalRef.current.classList.remove('is-animating');
      }
    };
